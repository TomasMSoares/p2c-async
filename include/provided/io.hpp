...
public:
  ...
  [[nodiscard]] bool is_backed() const {
    return handle != -1;
  }

  void open(const char *file, int flags, std::size_t size) {
    close();
    int h = -1;
    if (file) {
  ...
  [[nodiscard]] T *data() const { return mapping; }
  [[nodiscard]] const iterator begin() const { return data(); }
...
template <typename T> struct DataColumn : FileMapping<T> {
  ...
  DataColumn(size_t size, int flags = O_RDWR) : FileMapping<T>(size, flags), count(size / sizeof(T)) {
  ...
  [[nodiscard]] uintptr_t size() const { return count; }
...
struct DataColumn<std::string_view> : FileMapping<variable_size::StringData> {
  ...
  DataColumn(const std::string& filename, int flags = 0, uintptr_t size = 0) : DataColumn(filename.data(), flags, size) {}
  DataColumn(size_t size, int flags = O_RDWR) : FileMapping<Data>(size, flags) {}
  ...
  [[nodiscard]] Data *data() const { return this->mapping; }
  [[nodiscard]] uintptr_t size() const { return data()->count; }

  [[nodiscard]] const iterator begin() const { return iterator{0, data()}; }
  [[nodiscard]] const iterator end() const { return iterator{data()->count, data()}; }

  [[nodiscard]] variable_size::StringIndexSlot &slot_at(std::size_t idx) const {
