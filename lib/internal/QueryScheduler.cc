...
// SyncParquetScheduler

SyncParquetScheduler::SyncParquetScheduler(Database& db, size_t nthreads) : QueryScheduler(db), nthreads(nthreads) {}

void SyncParquetScheduler::execScanPipelineImpl(ScanPipeline& pipeline, QueryCompiler& qc) {
...
AsyncParquetScheduler::AsyncParquetScheduler(Database& db, const Config& cfg, size_t nthreads) :
    QueryScheduler(db),
    nthreads(nthreads),
...
AsyncParquetScheduler::~AsyncParquetScheduler() {
    if (debug_log.is_open()) debug_log.close();
}

void AsyncParquetScheduler::log(size_t thread_id, const std::string& msg) {
...
AsyncParquetScheduler::EpochStats::Snapshot AsyncParquetScheduler::EpochStats::takeSnapshot() {
    return Snapshot{
...
void AsyncParquetScheduler::execScanPipelineImpl(ScanPipeline& pipeline, QueryCompiler& qc) {
    auto fn = qc.getPipelineFunction(pipeline.name);
    if (!fn) llvm::report_fatal_error(fn.takeError());
...
void AsyncParquetScheduler::updateSchedulerStats() {
...
void AsyncParquetScheduler::prepareOneBlock(anyblob::network::Transaction& txn, ParquetTable::DownloadCallback cb) {
...
void AsyncParquetScheduler::processOneMorsel(ScanFn kernel_ptr, const std::vector<std::string>& layout_map, void** pipeline_args, size_t thread_id, std::shared_ptr<anyblob::network::TaskedSendReceiverHandle> io_handle) {
...
void AsyncParquetScheduler::driveIOHandle(std::shared_ptr<anyblob::network::TaskedSendReceiverHandle> handle) {
    auto start = std::chrono::steady_clock::now();
    handle->process(true);
    auto end = std::chrono::steady_clock::now();
...
AsyncParquetScheduler::JobType AsyncParquetScheduler::pickJob() {
